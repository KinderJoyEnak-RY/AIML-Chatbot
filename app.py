    ...
    if request.method == 'POST' and 'fullname' in request.form and 'username' in request.form and 'password' in request.form and 'email' in request.form:
       # Create variables untuk request username dan password
        ...
        if account:
            ...
            mycursor.execute('INSERT INTO user (fullname, username, password, email, created_at, updated_at) VALUES (%s, %s, %s, %s, now(), now())',
...
def edit_profile(id):
    if request.method == 'GET':
        ...
        mycursor.execute(''' UPDATE `user` SET fullname = %s, email = %s, username = %s, updated_at = now() WHERE id = %s ''',
...
def editpass_profile(id):
    if request.method == 'GET':
        ...
        mycursor.execute(
            ''' UPDATE `user` SET password = %s, updated_at = now() WHERE id = %s ''', (password, id, ))
        conn.commit()
        mycursor.close()
...
def add_tag():
    if request.method == 'GET':
        ...
        # insert data tag
        mycursor.execute(
            'INSERT INTO tag (tag, created_at, updated_at) VALUES (%s, now(), now())', (tag,))
        conn.commit()
...
def edit_tag(id):
    if request.method == 'GET':
        ...
        mycursor = conn.cursor()
        mycursor.execute(
            ''' UPDATE tag SET tag = %s WHERE id = %s ''', (tag, id))
        conn.commit()
...
def pattern_options(id):
    ...
    if request.method == 'GET':
        mycursor = conn.cursor()
        mycursor.execute(
            ''' SELECT id, pattern FROM pattern WHERE tag_id=%s''', (id, ))
        data = mycursor.fetchall()

        for item in data:
            result += "<option value='" + \
                str(item[0]) + "'>" + item[1] + "</option>"
...
def add_pattern():
    ...
    if request.method == 'GET':
        ...
        mycursor.execute(
            'SELECT * FROM pattern WHERE pattern = %s', (pattern,))
        ...
        if dataPattern:
            ...
            mycursor.execute(
                'INSERT INTO pattern (pattern, tag_id, created_at, updated_at) VALUES (%s, %s, now(), now())', (pattern.upper(), tagId,))
            conn.commit()
            # update aiml file
            update_aiml_file()
            mycursor.close()
...
def edit_pattern(id):
    if request.method == 'GET':
        ...
        mycursor = conn.cursor()
        mycursor.execute(
            ''' UPDATE pattern SET pattern = %s, tag_id = %s WHERE id = %s ''', (pattern.upper(), tagId, id, ))
        conn.commit()
        # update aiml file
        update_aiml_file()
...
def add_response():
    if request.method == 'GET':
        ...
        mycursor = conn.cursor()
        mycursor.execute(
            'INSERT INTO response (response, tag_id, pattern_id, created_at, updated_at) VALUES (%s, %s, %s, now(), now())', (response, tagId, patternId,))
        conn.commit()
        # update aiml file after create tag
        update_aiml_file()
...
def edit_response(id):
    if request.method == 'GET':
        ...
        mycursor = conn.cursor()
        mycursor.execute(''' UPDATE response SET response = %s, tag_id = %s, pattern_id = %s WHERE id = %s ''',
                         (response, tagId, patternId, id, ))
        conn.commit()
        # update aiml file
        update_aiml_file()
...
DEFAULT_RESPONSES = [
    "Maaf, saya tidak dapat memahami pertanyaan Anda🙏 mohon sertakan konteks/informasi dasar pada pertanyaan anda🙏"]
...
def get_bot_response():
    ...
    x = response.replace("((", "<").replace("))", ">").replace(
        "]", "").replace("'", "")
...
def preprocess(text):
    ...
    words = text.split()
    # Melakukan spell check dan stemming
    stemmed_words = [stemmer.stem(correction(word)) for word in words]
    ...
    print("Hasil spell check & stemming: ", stemmed_words)
